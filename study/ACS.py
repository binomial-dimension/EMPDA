...
class ACS_TSP:
    def __init__(self, func, n_dim,l_ini,
                ...
                alpha=1, beta=2, rho=0.1,seed =None
                ):
        self.func = func
        ...
        if l_ini != None:
            self.Tau[:] = 1.0/(self.n_dim*self.l_ini) # 如果有L_ini就用L_ini 确定fai_0
        ...
        for i in range(self.max_iter):  # 对每次迭代
            ...
            for j in range(self.size_pop):  # 每个蚂蚁
                for k in range(self.n_dim - 1):  # 每个节点
                    n1, n2 = self.Table[j, k], self.Table[j, k + 1]  # 蚂蚁从 n1 节点爬到 n2 节点
                    delta_tau[n1, n2] += (1/y_best - 1.0/(self.n_dim * self.l_ini))  # 涂抹的信息素
                n1, n2 = self.Table[j, self.n_dim - 1], self.Table[j, 0]  # 蚂蚁从最后一个节点爬回到第一个节点
                delta_tau[n1, n2] += (1/y_best - 1.0/(self.n_dim * self.l_ini)) # 涂抹信息素

            # 信息素飘散 + 信息素涂抹
            self.Tau = self.rho * self.Tau + (1-self.rho)*delta_tau
