...
class MMAS_TSP:
    def __init__(self, func, n_dim,l_ini,
        ...
        self.l_ini = l_ini #用于初始化信息素矩阵的L_ini
        self.seed = seed
        np.random.seed(seed) #用于设定seed
        self.prob_matrix_distance = 1 / (distance_matrix + 1e-10 * np.eye(n_dim, n_dim))  # 避免除零错误

        self.Tau = np.ones((n_dim, n_dim))  # 信息素矩阵，每次迭代都会更新
        self.Tau[:] = 1.0/((1-self.rho)*self.l_ini) # 就用L_ini 确定fai_0
        self.fai_max = 1.0/((1-self.rho)*self.l_ini)
        self.fai_min = self.fai_max/(2*n_dim)
    ...
    def run(self, max_iter=None):
        ...
        for i in range(self.max_iter):  # 对每次迭代
            ...
            for k in range(self.n_dim - 1): # 最优秀蚂蚁走过的每个节点
                n1,n2 = self.Table[index_best,k],self.Table[index_best,k + 1] # 最优蚂蚁从 n1 节点爬到 n2 节点
                delta_tau[n1, n2] += 1/y_best  # 涂抹的信息素
            n1, n2 = self.Table[index_best, self.n_dim - 1], self.Table[index_best, 0]  # 蚂蚁从最后一个节点爬回到第一个节点
            delta_tau[n1, n2] += 1 / y_best   # 涂抹信息素


            # 信息素飘散 + 信息素涂抹
            self.Tau = self.rho * self.Tau + delta_tau
            np.clip(self.Tau,self.fai_min,self.fai_max)
